%Tau-leap algorithm for our poliovirus model
function [Classes] = Tauleap_polio_model(para,ICs,maxtime,tau)
    ...
    %becomes negative and needs to be rectified
    Classes.NegativeCount = 0;
    ...
    while ((t<maxtime) && ( (Ia>0)||(Is>0)||(Vopv>0) ))
        ...
        r_4 = para.k*para.beta*Vopv*(Ia + Is)/para.N; %Infection after OPV vaccine
        r_5 = para.k*para.beta*Stil*(Ia + Is)/para.N; %Re-infection after infection
        r_6 = para.nu*S; %Vaccination
        r_7 = para.delta*Vopv; %Mutation of live attenuated virus in OPV to cVDPV
        r_8 = para.gamma*Is; %Recovery from virus that induced paralysis
        ...
        Vipv = Vipv - X_3 + X_6;
        Vopv = Vopv - X_4 - X_7;
        Stil = Stil - X_5 + X_8 + X_9;
        %Add all symptomatic infections to this counting class
        IsCount = IsCount + X_1;
        ...
        if Vopv<0 %"undo" infection OR mutation - both of these being 
                  %reverted decreases Ia so no random choice required.
            NegativeCount = NegativeCount + 1;
            tmp = Vopv;
            Vopv = 0;
    ...
    Classes.NegativeCount = [Classes.NegativeCount NegativeCount];
    end
